...
class RunningList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print(
        'RunningList重建了${++rebuildIndex}时间戳间隔${DateTime.now().millisecondsSinceEpoch - currentTimeStamp}ms');
...
Future<void> generateAndSum(
  ...
  print('lenght = $length iter.lenght = ${iter.length}');
  int sum = 0;
  int count = 1;
  ...
  for (int x in iter) {
    ...
    if (count % 1000000 == 0) {
      //如果count是一百万的整数
      print('执行了 count = $count');

      callerSP.send((count ~/ 1000000) * length); //就对外发送一次进度
    }
    count++;
  }
